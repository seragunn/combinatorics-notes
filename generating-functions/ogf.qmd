...
Let $L$ be a subset of strings. These is also called a **language**. Let $f$ be a metric. We define the **(ordinary) generating function** of $L$ or **OGF** as
...
::: {#exm-ogf1}
Let $L$ be the subset $\{01, 1110, 11, 0, 1, 111000, \varepsilon\}$ of binary strings. Let $f$ be the length metric. The OGF of $L$ is
\begin{align*}
...
b) write down the generating function with respect to the length metric
c) write down the generating function with respect to the "number of 1s" metric
:::
...
### When do we care about convergence?
For the most part, *we don't*. However, in more advanced courses, there are arguements using analysis which only apply to convergent power series. For instance, the fact that the series
$$
